        ...
        <div className="max-w-7xl mx-auto w-full">
          ...
          {showCreateModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 px-2 sm:px-0">
              <div className={`rounded-2xl shadow-2xl p-4 sm:p-8 w-full max-w-md relative animate-fade-in ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white'}` + ' max-h-[95vh] overflow-y-auto'}>
                <button
                  ...
                  title="Close"
                >×</button>
                <h2 className={`text-xl sm:text-2xl font-bold mb-4 flex items-center gap-2 ${theme === 'dark' ? 'text-blue-200' : 'text-blue-700'}`}><FaTshirt /> Create Uniform Request</h2>
                <form onSubmit={handleCreateRequest} className="space-y-4 sm:space-y-5">
                  <div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredRequests.map(request => (
                  <div key={request._id} className={`p-4 sm:p-5 rounded-2xl shadow-lg transition-all duration-300 ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}` + ' flex flex-col'}>
                    <div className="flex flex-col sm:flex-row items-center gap-4 mb-4">
                      <div className="w-16 h-16 rounded-full overflow-hidden mx-auto sm:mx-0">
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        <h3 className={`text-lg sm:text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{request.employee.fullName}</h3>
                        <p className={`text-xs sm:text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{request.employee.designation} - {request.employee.projectName}</p>
                      </div>
                    </div>
                    <div className="flex flex-col gap-2 mb-4">
                      <div className="flex flex-col sm:flex-row items-center justify-between">
                        <span className={`text-xs sm:text-sm font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Status:</span>
                        <span className={`text-xs sm:text-sm ${request.status === 'Approved' ? 'text-green-500' : request.status === 'Rejected' ? 'text-red-500' : 'text-yellow-500'}`}>{request.status}</span>
                      </div>
                      <div className="flex flex-col sm:flex-row items-center justify-between">
                        <span className={`text-xs sm:text-sm font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Items:</span>
              ...
              <div className="overflow-x-auto rounded-2xl shadow-lg">
                <table className={`min-w-full divide-y divide-gray-200 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} text-xs sm:text-sm`}>
                  <thead className={theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}>
                    <tr>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">{/* Checkbox for bulk actions */}</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Employee ID</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Name</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Designation</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Project</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Status</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Requested Items</th>
                      <th className="px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {filteredRequests.map(request => (
                      <tr key={request._id} className={theme === 'dark' ? 'bg-gray-800' : 'bg-white'}>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm font-medium text-gray-900">{/* Checkbox */}</td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm text-gray-500">{request.employee.employeeId}</td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm text-gray-500">{request.employee.fullName}</td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm text-gray-500">{request.employee.designation}</td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm text-gray-500">{request.employee.projectName}</td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm">
                          <span className={`px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold ${request.status === 'Approved' ? 'bg-green-100 text-green-700' : request.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                            {request.status}
                          </span>
                        </td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm">
                          <div className="flex flex-wrap gap-1 sm:gap-2">
                            {request.requestedItems.map(item => (
                              <span key={item} className={`inline-block rounded-full px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-semibold ${theme === 'dark' ? 'bg-blue-900 text-blue-100' : 'bg-blue-50 text-blue-700'}`}>
                                {item}
                              </span>
                            ))}
                          </div>
                        </td>
                        <td className="px-2 sm:px-6 py-4 whitespace-nowrap text-[10px] sm:text-sm font-medium">
                          <div className="flex flex-col sm:flex-row gap-1 sm:gap-2">
                            <button
                              onClick={() => handleAction(request._id, request.status === 'Approved' ? 'reject' : 'approve')}
                              className={`px-2 sm:px-4 py-2 rounded-lg font-semibold transition-all flex items-center justify-center gap-2 focus:outline-none ${request.status === 'Approved'
                                ? theme === 'dark'
                                  ? 'bg-red-600 text-white hover:bg-red-700'
                                  : 'bg-red-500 text-white hover:bg-red-600'
                                : theme === 'dark'
                            ...
                            <button
                              onClick={() => setRequests(prev => prev.filter(req => req._id !== request._id))}
                              className={`px-2 sm:px-4 py-2 rounded-lg font-semibold transition-all flex items-center justify-center gap-2 focus:outline-none ${theme === 'dark' ? 'bg-gray-800 text-white hover:bg-gray-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                            >
                              <FaTimesCircle /> Remove
                            </button>
                          </div>
