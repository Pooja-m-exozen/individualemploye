...
const handleCreateRequest = async (e: React.FormEvent) => {
  ...
  const startEdit = (mapping: Mapping) => {
    ...
    const selectedProject = projectList.find(p => p.projectName === mapping.project);
    if (selectedProject && selectedProject.designationWiseCount) {
      setDesignationOptions(Object.keys(selectedProject.designationWiseCount));
    } else {
      setDesignationOptions([]);
              ...
              <table className="w-full text-xs table-fixed border-separate" style={{ borderSpacing: 0 }}>
                <thead className={theme === "dark" ? "bg-blue-900 sticky top-0 z-10" : "bg-blue-50 sticky top-0 z-10"}>
                  <tr>
                    ...
                    <th className={`px-1 py-2 text-left font-bold uppercase whitespace-nowrap border ${theme === "dark" ? "text-blue-200 border-blue-800" : "text-blue-700 border-blue-200"}`} style={{ width: '10%' }}>Employee ID</th>
                    ...
                    <th className={`px-1 py-2 text-left font-bold uppercase whitespace-nowrap border ${theme === "dark" ? "text-blue-200 border-blue-800" : "text-blue-700 border-blue-200"}`} style={{ width: '10%' }}>Mapping Name</th>
                    <th className={`px-1 py-2 text-left font-bold uppercase whitespace-nowrap border ${theme === "dark" ? "text-blue-200 border-blue-800" : "text-blue-700 border-blue-200"}`} style={{ width: '7%' }}>Actions</th>
                  </tr>
                </thead>
                <tbody className={theme === "dark" ? "divide-y divide-blue-900" : "divide-y divide-blue-50"}>
                  {filteredMappings.map((mapping, idx) => (
                    <tr key={mapping._id} className={`${theme === "dark" ? "hover:bg-blue-900 transition" : "hover:bg-blue-50 transition"} even:bg-gray-50 dark:even:bg-gray-900`}>
                      <td className={`px-2 py-1 sticky left-0 z-10 font-mono text-[10px] border ${theme === 'dark' ? 'bg-gray-800 text-gray-300 border-blue-800' : 'bg-white text-gray-600 border-blue-200'}`}>{idx + 1}</td>
                      <td className={`px-2 py-1 border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}><div className="truncate" title={mapping.project}>{mapping.project}</div></td>
                      <td className={`px-2 py-1 border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}>
                        <div className="text-xs">
                          {mapping.designations.map((designation) => (
                      ...
                      <td className={`px-2 py-1 border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}><div className="truncate" title={mapping.employeeId || 'N/A'}>{mapping.employeeId || 'N/A'}</div></td>
                      <td className={`px-2 py-1 text-center border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}>
                        <span className={`inline-block text-xs font-semibold px-2 py-1 rounded-full ${
                          mapping.payable === 'payable' 
                            ? theme === 'dark' ? 'bg-green-800 text-green-200' : 'bg-green-100 text-green-700'
                            : theme === 'dark' ? 'bg-yellow-800 text-yellow-200' : 'bg-yellow-100 text-yellow-700'
                        }`}>
                          {mapping.payable}
                      ...
                      <td className={`px-2 py-1 border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}><div className="truncate" title={mapping.mappingName || 'N/A'}>{mapping.mappingName || 'N/A'}</div></td>
                      <td className={`px-2 py-1 text-center border ${theme === "dark" ? "border-blue-800" : "border-blue-200"}`}>
                        <div className="flex gap-1 justify-center">
                          <button
                          ...
                          <button
                            onClick={() => handleMapDelete(mapping._id)}
                  ...
                  <div className={`overflow-auto rounded-lg border ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        <tr>
                          ...
                          <td className="px-4 py-3">
                            <select
                              ...
                              onChange={(e) => setEditModalForm(prev => ({ ...prev, project: e.target.value }))}
                              required
                            >
                        ...
                        <tr>
                          <td className={`px-4 py-3 font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Employee ID</td>
                          <td className="px-4 py-3">
                            <input
                              type="text"
                              className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 ${theme === 'dark' ? 'bg-gray-800 text-blue-200 border-gray-600 focus:ring-blue-800' : 'bg-white text-blue-900 border-gray-300 focus:ring-blue-400'}`}
                              value={editModalForm.employeeId}
                              onChange={(e) => setEditModalForm(prev => ({ ...prev, employeeId: e.target.value }))}
                            ...
                            <div className="flex items-center gap-6">
                              <label className="flex items-center gap-2 cursor-pointer">
                                <input
                                  ...
                                  onChange={() => setEditModalForm(prev => ({ ...prev, payable: 'payable' }))}
                                  className="accent-green-600"
                                />
                                <span className={`font-semibold ${theme === 'dark' ? 'text-green-300' : 'text-green-700'}`}>Payable</span>
                              </label>
                              <label className="flex items-center gap-2 cursor-pointer">
                                <input
                                  ...
                                  onChange={() => setEditModalForm(prev => ({ ...prev, payable: 'non-payable' }))}
                                  className="accent-red-600"
                                />
                                <span className={`font-semibold ${theme === 'dark' ? 'text-red-300' : 'text-red-700'}`}>Non-Payable</span>
                              </label>
                        ...
                        <tr>
                          ...
                          <td className="px-4 py-3">
                            <input
                              ...
                              onChange={(e) => setEditModalForm(prev => ({ ...prev, mappingName: e.target.value }))}
                              placeholder="Optional mapping name..."
                            />
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td className="px-4 py-3">
                            <textarea
                              ...
                              onChange={(e) => setEditModalForm(prev => ({ ...prev, description: e.target.value }))}
                              placeholder="Optional description"
                              rows={2}
                            />
                          </td>
