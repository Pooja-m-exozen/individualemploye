  ...
  return (
    <div className={`min-h-screen font-sans ${theme === 'dark' ? 'bg-gradient-to-br from-gray-950 via-gray-900 to-blue-950' : 'bg-gradient-to-br from-indigo-50 via-white to-blue-50'}`}>
      <div className="p-2 md:p-6">
        {/* Header */}
        <div className={`rounded-2xl mb-4 md:mb-8 p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-5 shadow-lg ${theme === 'dark' ? 'bg-[#323a48]' : 'bg-gradient-to-r from-blue-500 to-blue-800'}`}>
          <div className={`${theme === 'dark' ? 'bg-[#232a36]' : 'bg-blue-600 bg-opacity-30'} rounded-xl p-3 md:p-4 flex items-center justify-center`}>
            <FaIdCard className="w-8 h-8 md:w-10 md:h-10 text-white" />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">KYC Report</h1>
            <p className={`text-base md:text-lg ${theme === 'dark' ? 'text-blue-200' : 'text-white opacity-90'}`}>View and export KYC details for employees.</p>
          </div>
        </div>
