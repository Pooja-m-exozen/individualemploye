            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className={`${theme === 'dark' ? 'bg-gray-900 text-gray-200' : 'bg-gray-50 text-gray-700'}`}>
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Issued Date</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className={`divide-y ${theme === 'dark' ? 'divide-gray-700' : 'divide-gray-200'} ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
                    {filteredCards.map((card, idx) => (
                      <tr key={card._id} className={`transition ${theme === 'dark' ? `${idx % 2 === 0 ? 'bg-gray-900' : 'bg-gray-800'} hover:bg-gray-700` : `${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50'}`}`}> 
                        ...
                        <td className={`px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{card.designation}</td>
                        <td className={`px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{card.projectName}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className={`px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{card.issuedDate ? new Date(card.issuedDate).toLocaleDateString() : 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center gap-4">
                            ...
                            <button className={`flex items-center gap-2 ${theme === 'dark' ? 'text-green-400 hover:text-green-200' : 'text-green-600 hover:text-green-800'} transition`}>
                              <FaDownload /> Download
                            </button>
                          </div>
                        </td>
