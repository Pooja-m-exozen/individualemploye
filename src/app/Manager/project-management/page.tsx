      ...
      <div className="p-6">
        ...
        <div className="flex justify-end mb-4">
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-lg shadow transition-colors flex items-center gap-2"
          >
            + Create Project
          </button>
        </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div ref={modalRef} className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-lg relative">
              <button
              ...
              <h2 className="text-2xl font-bold text-blue-700 mb-6">Create Project</h2>
              <form onSubmit={handleCreateProject} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Designation-wise Count</label>
                  {form.designationWiseCount.map((item, idx) => (
                    <div key={idx} className="flex gap-2 mb-2">
                      ...
                      <input
                        ...
                        placeholder="Count"
                        required
                        className="w-24 border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
        ...
        <div className="overflow-x-auto rounded-xl border border-blue-100 bg-white shadow-xl">
          <table className="min-w-full divide-y divide-blue-100">
            <thead className="bg-blue-50 sticky top-0 z-10">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase">Project Name</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase">Address</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase">Total Manpower</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase">Designation-wise Count</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-blue-700 uppercase">Last Updated</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-blue-50">
              {filteredProjects.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-gray-500">No projects found</td>
                </tr>
              ) : filteredProjects.map((project, idx) => (
                <tr key={idx} className="hover:bg-blue-50 transition">
                  <td className="px-4 py-3 font-bold text-blue-800">{project.projectName}</td>
