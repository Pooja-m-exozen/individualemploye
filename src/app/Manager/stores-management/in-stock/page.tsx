          ...
          <div className="lg:w-1/3 w-full">
            <div className="rounded-xl p-6 border bg-white border-blue-100 shadow-sm sticky top-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-900">Stock Guidelines</h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredStock.length === 0 ? (
                ...
                filteredStock.map((stock, idx) => (
                  <div key={idx} className="rounded-2xl border border-blue-100 bg-white shadow-lg flex flex-col overflow-hidden hover:shadow-xl transition-all">
                    <div className="relative w-full h-48 bg-blue-50 flex items-center justify-center">
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      <div>
                        <h3 className="text-xl font-bold text-blue-800 mb-1">{stock.item}</h3>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold">{stock.category}</span>
                        </div>
                        <div className="text-gray-600 text-sm mb-2">Quantity: <span className="font-semibold text-blue-700">{stock.quantity}</span></div>
                        <div className="text-gray-400 text-xs">Last Updated: {stock.lastUpdated}</div>
                      </div>
                      <div className="mt-6">
                        <button className="w-full px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 transition">View</button>
                      </div>
                    </div>
                  </div>
                ))
