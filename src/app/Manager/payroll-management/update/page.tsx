...
export default function PayrollUpdatePage() {
      ...
      try {
        ...
        if (attendanceData.attendance && attendanceData.attendance.length > 0) {
          const formattedAttendance: Attendance[] = attendanceData.attendance.map((att: unknown) => ({
            ...
            punchInTime: (att as { punchInTime?: string }).punchInTime ?? null,
            punchOutTime: (att as { punchOutTime?: string }).punchOutTime ?? null,
            status: (att as { status?: string }).status || "Present",
          }));
          setAttendanceData(formattedAttendance);
        } else {
          const mockAttendance: Attendance[] = [
            { date: "2025-06-01", punchInTime: "09:00:00", punchOutTime: "18:00:00", status: "Present" },
            { date: "2025-06-02", punchInTime: "08:45:00", punchOutTime: "17:45:00", status: "Present" },
            { date: "2025-06-03", punchInTime: null, punchOutTime: null, status: "Absent" },
            { date: "2025-06-04", punchInTime: "09:15:00", punchOutTime: "18:15:00", status: "Present" },
          ];
  ...
  const renderSelectEmployee = () => (
    <div className="space-y-6">
      <div
        className={`rounded-2xl shadow p-6 border flex flex-col gap-6 ${theme === "dark" ? "bg-gray-800 border-blue-900" : "bg-white border-blue-100"}`}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <input
            ...
            className={`w-full md:w-1/2 px-4 py-3 rounded-lg border ${theme === "dark"
          ...
          <button
            ...
            disabled={loading}
            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-colors ${theme === "dark"
  ...
  const renderUpdatePayroll = () => {
    ...
    return (
      <div className="space-y-8">
        <div
          className={`rounded-xl p-6 border shadow flex flex-col gap-4 ${theme === "dark" ? "bg-gray-900 border-gray-700" : "bg-white border-blue-100"}`}
        >
          ...
          {employeeDetails && (
            <div
              className={`mb-4 p-4 rounded-lg flex flex-col md:flex-row md:items-center gap-2 ${theme === "dark" ? "bg-blue-950 border border-blue-900" : "bg-blue-50 border border-blue-100"}`}
            >
          ...
          {payrollLoading && (
            <div className="text-blue-500 flex items-center gap-2">
              <FaSpinner className="animate-spin" /> Loading payroll details...
            </div>
          )}
          {payrollError && <div className="text-red-500">{payrollError}</div>}
          {payrollSuccess && <div className="text-green-600">{payrollSuccess}</div>}
          <form
            className="space-y-6"
            onSubmit={async (e) => {
              e.preventDefault();
              ...
              try {
                const res = await fetch(
                  `https://cafm.zenapi.co.in/api/salary-disbursement/employees/${selectedEmployee}/payroll-details`,
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
          ...
          >
            <div className="flex gap-4">
              <div className="flex-1">
                <label
                  className={`block text-sm font-medium mb-1 ${theme === "dark" ? "text-gray-200" : "text-black"}`}
                >
                  Month
                </label>
                <input
                  type="text"
                  className={`w-full px-4 py-2 rounded-lg border bg-gray-100 ${theme === "dark" ? "bg-gray-800 border-gray-700 text-gray-100" : "border-gray-200 text-black"}`}
                  value={new Date(0, month - 1).toLocaleString("default", { month: "long" })}
                  disabled
                />
              </div>
              <div className="flex-1">
            ...
            <div className="mt-6">
              <label
                className={`block text-sm font-medium mb-2 ${theme === "dark" ? "text-blue-300" : "text-blue-700"}`}
              >
                Attendance for {new Date(0, month - 1).toLocaleString("default", { month: "long" })} {year}
              </label>
              <div className="overflow-x-auto">
                <table
                  className={`w-full rounded-lg shadow-md overflow-hidden ${theme === "dark" ? "bg-gray-900" : "bg-white"}`}
                >
                  <thead>
                    <tr>
                      {attendanceRow.map((_, i) => (
                        <th
                          key={i}
                          className={`p-2 text-xs font-semibold text-center border-b ${theme === "dark" ? "text-gray-400 border-gray-700" : "text-gray-500 border-b"}`}
                        >
                          {i + 1}
                        </th>
                      ))}
                      <th
                        className={`p-2 text-xs font-semibold text-center border-b ${theme === "dark" ? "text-blue-300 border-gray-700" : "text-blue-700 border-b"}`}
                      >
                        Total Payable
                      </th>
                    </tr>
                    ...
                    <tr>
                      {attendanceRow.map((cell, i) => {
                        ...
                        return (
                          <td
                            key={i}
                            className={`p-2 text-center font-bold ${cellClass} ${theme === "dark" ? "bg-opacity-80" : ""}`}
                          >
                            {cell.status}
                          </td>
                        );
                      ...
                      <td
                        className={`p-2 text-center font-bold ${theme === "dark" ? "text-blue-200 bg-blue-950" : "text-blue-700 bg-blue-50"}`}
                      >
                        {totalPayable}
                      </td>
                    </tr>
                ...
                <div className="flex flex-wrap gap-4 mt-4 items-center">
                  <span
                    className={`flex items-center gap-2 ${theme === "dark" ? "" : "text-black"}`}
                  >
                  ...
                  <span
                    className={`flex items-center gap-2 ${theme === "dark" ? "" : "text-black"}`}
                  >
                    <span className="w-4 h-4 rounded bg-yellow-400 border border-yellow-600 inline-block"></span>
                    Holiday (H)
                  </span>
                </div>
              </div>
            ...
            <div className="flex justify-between">
              <button
                type="button"
                className={`px-8 py-3 rounded-xl font-medium border ${theme === "dark" ? "text-blue-300 border-blue-900 bg-blue-950 hover:bg-blue-900" : "text-blue-600 border-blue-200 bg-blue-50 hover:bg-blue-100"}`}
  ...
  return (
    <ManagerDashboardLayout>
      <div
        className={`min-h-screen flex flex-col ${theme === "dark" ? "bg-gradient-to-br from-gray-950 via-gray-900 to-blue-950" : "bg-gradient-to-br from-indigo-50 via-white to-blue-50"}`}
      >
        <div className="max-w-7xl mx-auto">
          <table
            className={`w-full text-sm min-w-[1200px] table-fixed ${theme === "dark" ? "bg-gray-900" : "bg-white"}`}
          >
            <thead className={theme === "dark" ? "bg-gray-800" : "bg-blue-50"}>
              <tr>
                <th
                  className={`px-3 py-2 text-left font-semibold ${theme === "dark" ? "text-gray-200" : "text-blue-700"} w-1/6`}
                >
                  Section
                </th>
                <th
                  className={`px-3 py-2 text-left font-semibold ${theme === "dark" ? "text-gray-200" : "text-blue-700"} w-5/6`}
                >
                  Content
                </th>
              </tr>
            ...
            <tbody>
              ...
              <tr className={theme === "dark" ? "bg-gray-900" : "bg-white"}>
                ...
                <td className="px-3 py-2 w-5/6">
                  <div
                    className={`flex items-center gap-6 rounded-2xl px-8 py-8 ${theme === "dark" ? "bg-[#323a48]" : "bg-gradient-to-r from-blue-600 to-blue-500"}`}
                  >
                    <div
                      className={`flex items-center justify-center w-16 h-16 rounded-xl ${theme === "dark" ? "bg-[#232a36]" : "bg-blue-500 bg-opacity-30"}`}
                    >
                    ...
                    <div>
                      ...
                      >
                        Update payroll details and view attendance for employees
                      </p>
                    </div>
                  </div>
                </td>
              ...
              {/* Stepper and Instructions Section */}
              <tr className={theme === "dark" ? "bg-gray-900" : "bg-white"}>
                <td className="px-3 py-2 w-5/6">
                  <div
                    className={`lg:w-96 w-full lg:h-auto h-28 flex flex-col gap-4 p-4 border-r shadow-lg ${theme === "dark" ? "bg-gray-900 border-gray-800" : "bg-white border-blue-100"}`}
                  >
                    <div
                      className={`rounded-xl p-5 flex items-start gap-4 border ${theme === "dark" ? "bg-[#232a36] border-gray-800" : "bg-blue-50 border-blue-100"}`}
                    >
                      <div
                        className={`p-3 rounded-xl flex items-center justify-center ${theme === "dark" ? "bg-[#323a48]" : "bg-blue-100"}`}
                      >
                        <FaInfoCircle
                          className={`w-6 h-6 ${theme === "dark" ? "text-blue-200" : "text-blue-600"}`}
                        />
                      </div>
                      <div>
                        <h3
                          className={`font-semibold mb-1 ${theme === "dark" ? "text-blue-200" : "text-blue-700"}`}
                        >
                          Instructions
                        </h3>
                        <ul
                          className={`list-disc ml-5 text-sm space-y-1 ${theme === "dark" ? "text-blue-200" : "text-blue-800"}`}
                        >
                    ...
                    <div className="flex lg:flex-col flex-row gap-2">
                      {steps.map((step, idx) => {
                        ...
                        return (
                          <button
                            key={step.id}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl w-full transition-colors font-medium text-left ${isActive
                              ? theme === "dark"
                          ...
                          >
                            ...
                            {isCompleted && (
                              <FaCheckCircle
                                className={`w-4 h-4 ml-auto ${theme === "dark" ? "text-green-300" : "text-green-500"}`}
                              />
                            )}
                          </button>
                        );
